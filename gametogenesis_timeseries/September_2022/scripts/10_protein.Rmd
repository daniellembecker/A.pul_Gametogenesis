...
prot_path = "September_2022/data/10_protein/10_host/"                                              # Path to prot data directory
...
df_host <- tibble(file = prot_data_files) %>%
  ...
  dplyr::mutate(platemap = map(plate, ~read_csv(paste0(prot_path, ., "_BCA_host_platemap.csv"))),
...
```{r}
# Surface area data
sa <- read.csv("September_2022/output/10_surface_area.calc.csv")

# Tissue homogenate volume data
homog_vols <- read_csv("September_2022/data/10_homogenate_vols/10_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("September_2022/10_corals_sampled.csv") %>% select(1:5)
...
prot_host %>%
  ...
  mutate(timepoint="SEPTEMBER")%>%
  write_csv(., path = "September_2022/output/10_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "September_2022/data/10_protein/10_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
prot_holobiont %>%
  ...
  select(colony_id, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="SEPTEMBER")%>%
  write_csv(., path = "September_2022/output/10_holobiont_protein.csv")
```
