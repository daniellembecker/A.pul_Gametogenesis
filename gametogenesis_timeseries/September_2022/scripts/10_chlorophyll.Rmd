...
chl_path <- "September_2022/data/10_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("September_2022/data/10_homogenate_vols/10_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("September_2022/output/10_surface_area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "September_2022/output/10_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("September_2022/10_corals_sampled.csv")
