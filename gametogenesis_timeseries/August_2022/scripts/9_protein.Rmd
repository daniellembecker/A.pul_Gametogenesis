...
prot_path = "August_2022/data/9_protein/9_host/"                                              # Path to prot data directory
...
mod_host <- nls(formula = BSA_ug.mL ~ z + a * exp(b * abs562), start = list(z = 0, a = 2, b = 2), data = std_curve_host)
...
metadata <- read_csv("August_2022/9_corals_sampled.csv") %>% select(1:5)
...
prot_host %>%
  ...
  write_csv(., path = "August_2022/output/9_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "August_2022/data/9_protein/9_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
prot_holobiont %>%
  ...
  select(colony_id, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="AUGUST")%>%
  write_csv(., path = "August_2022/output/9_holobiont_protein.csv")
```
