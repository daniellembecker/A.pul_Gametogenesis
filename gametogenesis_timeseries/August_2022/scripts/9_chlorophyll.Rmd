...
chl_path <- "August_2022/data/9_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("August_2022/data/9_homogenate_vols/9_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("August_2022/output/9_surface_area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "August_2022/output/9_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("August_2022/9_corals_sampled.csv")
