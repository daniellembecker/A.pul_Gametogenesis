...
chl_path <- "January_2022/data/2_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("January_2022/data/2_homogenate_vols/2_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("January_2022/output/2_surface.area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "January_2022/output/2_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("January_2022/2_corals_sampled.csv")
