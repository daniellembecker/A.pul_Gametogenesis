...
chl_path <- "October_2022/data/11_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("October_2022/data/11_homogenate_vols/11_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("October_2022/output/11_surface_area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "October_2022/output/11_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("October_2022/11_corals_sampled.csv")
