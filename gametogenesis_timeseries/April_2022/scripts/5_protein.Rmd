---
title: "APRIL 2022 host and holobiont protein analysis"
...
prot_path = "April_2022/data/5_protein/5_host/"                                              # Path to prot data directory
...
```{r}
# Surface area data
sa <- read.csv("April_2022/output/5_surface_area.calc.csv")

# Tissue homogenate volume data
homog_vols <- read_csv("April_2022/data/5_homogenate_vols/5_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("April_2022/5_corals_sampled.csv") %>% select(1:5)
...
prot_host %>%
  ...
  mutate(timepoint="APRIL")%>%
  write_csv(., path = "April_2022/output/5_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "April_2022/data/5_protein/5_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
prot_holobiont %>%
  ...
  select(colony_id, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="APRIL")%>%
  write_csv(., path = "April_2022/output/5_holobiont_protein.csv")
```
