...
chl_path <- "April_2022/data/5_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("April_2022/data/5_homogenate_vols/5_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("April_2022/output/5_surface_area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "April_2022/output/5_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("April_2022/5_corals_sampled.csv")
