...
chl_path <- "February_2022/data/3_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("February_2022/data/3_homogenate_vols/3_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("February_2022/output/3_surface.area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "February_2022/output/3_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("February_2022/3_corals_sampled.csv")
chl <- right_join(chl, meta, by = "colony_id")

# Plot chlorophyll a
chl %>%
  ggplot(aes(x = timepoint , y = chla.ug.cm2, color = timepoint)) +
  #facet_wrap(~species) +
...
# Plot chlorophyll c2
chl %>%
  ggplot(aes(x = timepoint, y = chlc2.ug.cm2, color = timepoint)) +
  #facet_wrap(~species) +
