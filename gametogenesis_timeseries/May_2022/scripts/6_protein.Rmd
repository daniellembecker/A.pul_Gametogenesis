---
title: "May 2022 host and holobiont protein analysis"
...
prot_path = "May_2022/data/6_protein/6_host/"                                              # Path to prot data directory
...
```{r}
# Surface area data
sa <- read.csv("May_2022/output/6_surface_area.calc.csv")

# Tissue homogenate volume data
homog_vols <- read_csv("May_2022/data/6_homogenate_vols/6_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("May_2022/6_corals_sampled.csv") %>% select(1:5)
...
prot_host %>%
  ...
  mutate(timepoint="MAY")%>%
  write_csv(., path = "May_2022/output/6_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "May_2022/data/6_protein/6_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
prot_holobiont %>%
  ...
  select(colony_id, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="MAY")%>%
  write_csv(., path = "May_2022/output/6_holobiont_protein.csv")
```
