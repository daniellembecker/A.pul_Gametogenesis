...
chl_path <- "May_2022/data/6_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("May_2022/data/6_homogenate_vols/6_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("May_2022/output/6_surface_area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "May_2022/output/6_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("May_2022/6_corals_sampled.csv")
