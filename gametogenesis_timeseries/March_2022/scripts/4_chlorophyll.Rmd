...
chl_path <- "March_2022/data/4_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("March_2022/data/4_homogenate_vols/4_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("March_2022/output/4_surface_area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "March_2022/output/4_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("March_2022/4_corals_sampled.csv")
...
chl %>%
  ...
  stat_summary(fun.y = mean, geom = "point", color 
               = "black")           # Plot mean
