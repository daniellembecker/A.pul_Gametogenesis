...
chl_path <- "July_2022/data/8_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("July_2022/data/8_homogenate_vols/8_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("July_2022/output/8_surface_area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "July_2022/output/8_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("July_2022/8_corals_sampled.csv")
