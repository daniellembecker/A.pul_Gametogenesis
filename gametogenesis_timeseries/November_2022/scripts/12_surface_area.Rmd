...
#load wax data
wax.data <- read.csv("October_2022/data/11_surface_area/11_surface_area_data.csv", header=TRUE)
...
summary(stnd.curve)$r.squared #r-squared = 0.9854
...
smpls%>%
  mutate(timepoint="OCT")%>%
  select(colony_id, surface.area.cm2, timepoint)%>%
    write_csv("October_2022//output/11_surface_area.calc.csv")
```
