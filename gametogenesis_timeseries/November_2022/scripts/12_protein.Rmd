...
prot_path = "July_2022/data/8_protein/8_host/"                                              # Path to prot data directory
...
```{r}
# Surface area data
sa <- read.csv("July_2022/output/8_surface_area.calc.csv")

# Tissue homogenate volume data
homog_vols <- read_csv("July_2022/data/8_homogenate_vols/8_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("July_2022/8_corals_sampled.csv") %>% select(1:5)
...
prot_host %>%
  ...
  mutate(timepoint="July")%>%
  write_csv(., path = "July_2022/output/8_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "July_2022/data/8_protein/8_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
prot_holobiont %>%
  ...
  select(colony_id, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="JULY")%>%
  write_csv(., path = "July_2022/output/8_holobiont_protein.csv")
```
