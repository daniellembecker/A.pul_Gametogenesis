...
oogen_model <- vglm(Stage ~ Host_AFDW.mg.cm2 + Sym_AFDW.mg.cm2 + 
              host_prot_mg.cm2 + chl_cells_combined,
...
coef_df_oogen <- coef_df_oogen %>%
  ...
  mutate(
    ...
    variable = case_when(
      variable == "chl_cells_combined" ~ "Symbiont and Chlorophyll Combined",
  ...
  # Reorder variables
  mutate(variable = factor(variable, levels = c("Symbiont and Chlorophyll Combined", "Symbiont Biomass", "Host Biomass", "Host Protein")))

# Create the plot
oogen_plot <- ggplot(coef_df_oogen, aes(x = factor(stage), y = coef, fill = variable)) +
...
phys_vars <- c("Host_AFDW.mg.cm2", "Sym_AFDW.mg.cm2", "host_prot_mg.cm2", "chl_cells_combined")
...
sperm_model <- vglm(Stage ~ Host_AFDW.mg.cm2 + Sym_AFDW.mg.cm2 + 
              host_prot_mg.cm2 + chl_cells_combined,
              family = cumulative(parallel = FALSE, reverse = TRUE),
...
sperm_plot <- ggplot(coef_df_sperm, aes(x = factor(stage), y = coef, fill = variable)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  geom_vline(xintercept = 1.5:5.5, color = "gray50", linetype = "solid") +
  scale_fill_brewer(palette = "Dark2") +
