...
```{r}
#load histo data
oogen_abundance <- read.csv("gametogenesis_timeseries/gametogenesis_timeseries_analysis/Output/oocyte_abun_data.csv")[-1]
  
sperm_abundance <- read.csv("gametogenesis_timeseries/gametogenesis_timeseries_analysis/Output/sperm_abun_data.csv")[-1]

oogen_size <- read.csv("gametogenesis_timeseries/gametogenesis_timeseries_analysis/Output/oogen_size_data.csv")[-1]
  
sperm_size <- read.csv("gametogenesis_timeseries/gametogenesis_timeseries_analysis/Output/sperm_size_data.csv")[-1]
...
temp.dat <- read.csv("gametogenesis_timeseries/gametogenesis_timeseries_analysis/Output/mean_temp_data.csv")[, -1]
...
light_summary_plot <- light.dat %>%
  ...
  ylab(expression("Mean Light Intensity " * (mu*mol~m^{-2}~s^{-1}))) +
  xlab("Date") +
  scale_x_datetime(date_breaks = "1 months", date_labels = "%b %Y") +
  theme_classic() + 
  theme(
...
y_limits <- list(
  temp = c(25, 30),
  light = c(1000, 6000),
  sym = c(min(summary_data$Symbiont_Density_mean - summary_data$Symbiont_Density_se, na.rm = TRUE),
          max(summary_data$Symbiont_Density_mean + summary_data$Symbiont_Density_se, na.rm = TRUE)),
  chl = c(min(summary_data$Total_Chlorophyll_mean - summary_data$Total_Chlorophyll_se, na.rm = TRUE),
          max(summary_data$Total_Chlorophyll_mean + summary_data$Total_Chlorophyll_se, na.rm = TRUE)),
  sym_bio = c(min(summary_data$Symbiont_Biomass_mean - summary_data$Symbiont_Biomass_se, na.rm = TRUE),
              max(summary_data$Symbiont_Biomass_mean + summary_data$Symbiont_Biomass_se, na.rm = TRUE))
)

# Apply consistent y-axis limits to each plot
temp_summary_plot <- temp_summary_plot +
  ylim(y_limits$temp)

light_summary_plot <- light_summary_plot +
  ylim(y_limits$light)
...
sym.bio <- sym.bio +
  ylim(y_limits$sym_bio)

# Combine the adjusted plots
combined_plot <- (temp_summary_plot / 
                   light_summary_plot / 
                   sym / 
                   chl / 
                   sym.bio) +
  plot_layout(heights = c(1, 1, 1, 1, 1)) # Adjust heights if needed
