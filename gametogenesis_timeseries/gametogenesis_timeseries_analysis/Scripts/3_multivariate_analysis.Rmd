...
phys_data <-master%>%
  ...
  filter(chlc2.ug.cm2>0)%>%
  filter(chla.ug.cm2>0)%>%
...
cor.df <- phys_data %>% dplyr::select(3:10) %>% na.omit()

# Save the correlation matrix plot as a PNG file with the absolute path
png(file = "gametogenesis_timeseries_analysis/Figures/correlation-matrix.png", height = 1400, width = 1400)
