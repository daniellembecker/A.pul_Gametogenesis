...
phys_data <-master%>%
  ...
  filter(holobiont_prot_mg.cm2>0)%>%
  filter(host_prot_mg.cm2>0)%>%
  dplyr::rename(Host_Biomass=Host_AFDW.mg.cm2, Symbiont_Biomass=Sym_AFDW.mg.cm2, Symbiont_Density=cells.cm2, Host_Protein=host_prot_mg.cm2, Holobiont_Protein=holobiont_prot_mg.cm2, Total_Chlorophyll=Total_Chl_cm2)
...
repo_wide_data <- repo_data %>%
  pivot_wider(
    id_cols = colony_id,
    names_from = c(timepoint, Stage, gonad.type),
    values_from = geometric.mean,
    values_fill = list(geometric.mean = 0)) %>%
  arrange(colony_id)
...
cor.df <- phys_data %>% dplyr::select(3:8) %>% na.omit()

# Save the correlation matrix plot as a PNG file with the absolute path
png(file = save_path, height = 1400, width = 1400)
corrplot(cor(cor.df), addCoef.col = "black", # Specify color for coefficients
...
corrplot(cor(cor.df), tl.col = "black", 
         method = 'circle', col = colorRampPalette(c("red", "white", "blue"))(200), 
