...
# Define a threshold for identifying outliers for the 'Value' column
threshold <- 7  # For example, we consider values greater than 50 as outliers

# Filter out values beyond the threshold for the 'Value' column
filtered_tot <- master[master$Total_Chl_cm2 <= threshold, ]

# Get the maximum value of tot chl
max_tot <- max(filtered_tot$Total_Chl_cm2, na.rm = TRUE)  # Make sure to handle any missing values

# Make chlc plot
tot.chl <- filtered_tot %>%
...
phys_host_figure<-plot_grid(host_prot_cm2, host_bio_cm2, ncol=2, nrow=1, labels = c('A', 'B'),label_size = 80,rel_heights= 1, rel_widths = 1, label_y=1, align="h")
...
#make plot of all 
phys_all_figure<-plot_grid(symbplot_cm2, sym_bio_cm2, tot.chl, host_prot_cm2, holobiont_prot_cm2, host_bio_cm2, ncol=2, nrow=3, labels = c('A', 'B', 'C', 'D', 'E', 'F'),label_size = 80,rel_heights= 1, rel_widths = 1, label_y=1, align="h")
...
#add to plot
phys_all_figure <- grid.arrange(arrangeGrob(phys_all_figure, bottom = x.grob))

ggsave(filename="gametogenesis_timeseries_analysis/Figures/Phys_all_Figure.png", plot=phys_all_figure, dpi=300, width=40, height=48, units="in")
```
