...
homogenate_files<-list.files("../gametogenesis_timeseries/", pattern = "homogenate_vols.csv", recursive=T, full.names=T)
...
```{r}
#homogenate files 
homogenate_dataset <- data.frame()

for (i in 1:length(homogenate_files)){
  homogenate_df <- read.csv(homogenate_files[i]) #each file will be read in
  homogenate_dataset <- rbind(homogenate_dataset, homogenate_df) #for each iteration, bind the new data to the building dataset
}

# Test if differences in homogenates, looking into June chlorophyll issue
# One-way ANOVA
anova_result <- aov(homog_vol_ml ~ timepoint, data = homogenate_dataset)
...
# differences in June compared to other months but no large outliers
