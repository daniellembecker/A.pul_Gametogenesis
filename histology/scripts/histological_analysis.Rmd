...
```{r}
# Replace NA in Stage with "No Data in Slides" if desired
abundance_clean <- abundance %>%
  mutate(Stage = ifelse(is.na(Stage), "No Data in Slides", as.character(Stage)))
...
df_relative_abundance_with_no_data <- rel_full %>%
  group_by(timepoint) %>%
  mutate(freq_no_data = 1 - sum(freq[Stage != "No Data in Slides"], na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(freq = ifelse(Stage == "No Data in Slides", freq_no_data, freq)) %>%
...
color_gradient <- c("Stage I" = "#EAE2F8", "Stage II" = "#D0A6F7", "Stage III" = "#B76CF7", "Stage IV" = "#8E2BEA", "Stage V" = "#3B1E8F", "No Data in Slides" = "#D3D3D3")
...
oocytes_plot <- df_relative_abundance_with_no_data %>%
  ...
  theme(axis.text.x = element_blank(),  # Remove x-axis text from oocytes plot
        ...
        legend.position = "none",  # Remove legend (it will be added globally)
...
# Ensure all stages including "No Data in Slides" are present
...
# Plot for Spermatocytes with gradient color scheme
spermatocytes_plot <- sperm_abun %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 20),  # Rotate x-axis labels 45 degrees and increase size
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +  # Remove grid lines for clarity
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1)); spermatocyte_plot


# Save the spermatocyte plot
ggsave("histology/output/spermatocyte_relative_abundance.png", spermatocyte_plot, width = 13, height = 8)
...
combined_plot <- (oocytes_plot / spermatocytes_plot) +
  ...
  plot_layout(guides = "collect") &  # Collect legends into a single legend above both plots
  theme(legend.position = "top");combined_plot  # Collect legends into a single legend
...
oogen <- oogen_size %>%
  ...
  theme(
    ...
    axis.title.y = element_text(face = "bold", size = 30),  # Keep y-axis title
    axis.text.y = element_text(size = 30, color = "black")); oogen
