...
# save oogenesis plot
ggsave("../output/oogenesis_abundance_percolony.png", oogen2 , width = 13, height = 8)
...
# save oogenesis plot
ggsave("../output/spermatogenesis_abundance_bycolony.png", spermato2 , width = 13, height = 8)
...
rel.abun <- abundance_clean %>%
  filter(gonad.type == "oocyte") %>%
  count(site, timepoint, Stage, gonad.type, colony_id, name = "stage_count") %>%
  group_by(timepoint) %>%
...
ggsave("../output/oocytes_relative_abundance.png", oocytes_plot, width = 13, height = 8)
...
rel_full_spermatocyte <- rel.abun_spermatocyte %>%
  complete(timepoint, Stage = c(levels(rel.abun_spermatocyte$Stage)), fill = list(freq = 0)) %>%
  group_by(timepoint) %>%
...
ggsave("../output/spermatocyte_relative_abundance.png", spermatocytes_plot, width = 13, height = 8)
...
ggsave("../output/combined_abundance_plots.png", combined_plot, width = 20, height = 15)
```
...
ggsave("../output/oogen.size.month.boxplot.png", oogen, width = 13, height = 8)
...
# Save the plot as an image
ggsave("../output/sperm.size.month.boxplot.png", sperm, width = 13, height = 8)
...
# Save the combined plot
ggsave("../output/combined_oocyte_sperm_size_plots.png", combined_plot, width = 20, height = 20)
```


# Calculate relative abundance change overtime with logistic regression, with colony_id as a random effect
```{r}
# Use abundance clean dataframe for oocytes and spermmaries
oocyte_data <- oocyte.abund.all
spermatocyte_data <- sperm_abun

# check beta regression model assumptions and inclusion of colony_id as a random effect
# For oocytes
model_oocyte_no_re <- betareg(freq_adjusted ~ Stage, data = oocyte_data)
model_oocyte_re <- glmmTMB(freq_adjusted ~ Stage + (1 | colony_id), family = beta_family(), data = oocyte_data)

multinom_model <- multinom(Stage ~ timepoint, data = oocyte_data, weights = freq)
