...
oogen <- oogen_df_relative_abundance %>% 
  ggplot(aes(x = replicate_id, y = freq, group = Stage, color = Stage, fill = Stage)) +
  geom_col() +
  geom_text(data = unique(oogen_df_relative_abundance[, c("replicate_id", "grand_tots")]), 
            aes(x = replicate_id, y = -0.5, label = grand_tots), 
            vjust = 0, color = "black", size = 3) +  # Add unique grand_tots numbers on x-axis
