...
```{r}
month_levels <- c("Dec 2021","Jan 2022","Feb 2022","Mar 2022","Apr 2022","May 2022",
...
env_z <- gamete_monthly %>%
  left_join(temp_monthly %>% select(month, temp_z = mean_temp), by = "month") %>%
  left_join(mcr_monthly %>% select(month, wind_z = mean_wind), by = "month") %>%
  left_join(light_monthly %>% select(month, light_z = mean_light), by = "month") %>%
  left_join(photoperiod_monthly %>% select(month, photo_z = mean_photoperiod), by = "month")
...
(p_temp | p_wind) / (p_light | p_photo) / p_bottom
```
