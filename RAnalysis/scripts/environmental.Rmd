...
# Calcualter z scores from the various files and variables
...
```{r}
# use a rescaling transofrmation to have z scores on the left and percentages for gametes on the right
...
month_scaffold <- tibble(month = month_levels)

env_z <- month_scaffold %>%
  left_join(gamete_monthly, by = "month") %>%
  left_join(temp_monthly %>% select(month, temp_z), by = "month") %>%
  left_join(mcr_monthly2 %>% select(month, wind_z), by = "month") %>%
  left_join(light_monthly %>% select(month, light_z), by = "month") %>%
  left_join(photoperiod_monthly %>% select(month, photo_z), by = "month") %>%
...
bars_long <- env_z %>%
  select(month, month_num, eggs_pct, sperm_pct) %>%
  tidyr::pivot_longer(
    cols = c(eggs_pct, sperm_pct),
    names_to = "type",
    values_to = "pct"
  ) %>%
  filter(!is.na(pct)) %>%          # <- drop truly missing months instead of plotting them as 0
  mutate(
    type = recode(type, eggs_pct = "Eggs", sperm_pct = "Sperm"),
    value = z_min + (pct / 100) * (z_max - z_min),
    offset = ifelse(type == "Eggs",
                     -(bar_width / 2 + gap / 2),
                      (bar_width / 2 + gap / 2)),
    xmin = month_num + offset - bar_width / 2,
    xmax = month_num + offset + bar_width / 2
  )

p_bottom <- ggplot() +
  geom_rect(
    data = bars_long,
    aes(xmin = xmin, xmax = xmax, ymin = z_min, ymax = value, fill = type),  # <- explicit baseline
    color = NA
  ) +
  geom_line(data = env_z2, aes(x = month_num, y = temp_z,  color = "Temperature", group = 1), linewidth = 1) +
  geom_line(data = env_z2, aes(x = month_num, y = wind_z,  color = "Wind speed",  group = 1), linewidth = 1) +
  geom_line(data = env_z2, aes(x = month_num, y = light_z, color = "Light",       group = 1), linewidth = 1) +
  geom_line(data = env_z2, aes(x = month_num, y = photo_z, color = "Photoperiod", group = 1), linewidth = 1) +
  scale_x_continuous(
    breaks = 1:12,
    labels = month_levels,
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ (. - z_min) / (z_max - z_min) * 100,
                        name = "% individuals with gametes")
  ) +
