...
ggplot(mean_monthly_data, aes(x = month)) +
  ...
  geom_point(aes(y = mean_light / 120, color = "Light Intensity", group = 1), size = 3) +  # Add points for light intensity
  geom_line(aes(y = mean_light / 120, color = "Light Intensity", group = 1)) +  # Add lines for light intensity
  scale_y_continuous(
    sec.axis = sec_axis(~ . * 120, name = "Light Intensity") , 
    limits = c(12, 29)) + # Set limits for the primary y-axis)) 
