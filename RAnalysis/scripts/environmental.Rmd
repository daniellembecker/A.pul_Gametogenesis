...
ggplot(mean_monthly_data, aes(x = month)) +
  ...
  geom_point(aes(y = mean_light, color = "Light Intensity", group = 1), size = 3) +  # Add points for light intensity
  geom_line(aes(y = mean_light, color = "Light Intensity", group = 1)) +  # Add lines for light intensity
  scale_y_continuous(
    name = "Temperature",
    limits = c(26, 29),  # Set limits for the primary y-axis
    sec.axis = sec_axis(~ ., name = "Light Intensity", 
                        breaks = seq(2000, 4500, by = 500),
                        labels = seq(2000, 4500, by = 500))) +  # Labels for secondary y-axis
  scale_color_manual(values = c("Temperature" = "black", "Light Intensity" = "grey")) +  # Set colors
  labs(x = "Month", y = NULL, ysec = NULL) +  # Set labels for axes
  theme_classic() +
  theme(
